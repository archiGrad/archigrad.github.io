...
def parse_custom_processing(path, current_config):
    ...
    if custom_file.exists():
        ...
        if parsed_keys:
            print(f"  .custom_processing LOADED from {path}: {len(parsed_keys)} keys")
            ...
            if mismatches:
                print(f"  TYPE MISMATCH in {path}: {mismatches}")
        else:
            print(f"  .custom_processing in {path}: file exists but 0 keys parsed")
...
def scan_folder(path, parent_hidden=False, inherited_spacing=None, inherited_zoom=None, inherited_keywords=None, inherited_reverse=None, inherited_sprite_bg=None, inherited_sprite_bg_opacity=None, inherited_sprite_bg_crop=None, inherited_sprite_bg_crop_opacity=None, ignore=None):
    ...
    if path.is_dir():
        ...
        if texts and images:
            print(f"  HTML-sibling images excluded from processing in {path}: {[Path(p).name for p in images]}")
            images = []
...
print(f"TARGET_ROOT = {TARGET_ROOT!r} -> exists={target_root.exists()}, is_dir={target_root.is_dir() if target_root.exists() else 'N/A'}")
...
print(f"Generated spritesheets, data.json and index.html in {int(_elapsed // 60)}m {int(_elapsed % 60)}s")
