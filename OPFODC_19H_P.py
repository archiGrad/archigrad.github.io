...
def log(level, msg):
    ...
    if level == 'WARN': _counts['warnings'] += 1
    elif level == 'ERR': _counts['errors'] += 1
    print(prefix + msg)
...
def parse_custom_processing(path, current_config):
    ...
    if custom_file.exists():
        ...
        try:
            ...
            log('WARN', f"E001 {path}: {e}")
        if parsed_keys:
            if VERBOSE:
                log('INFO', f".custom_processing loaded {len(parsed_keys)} keys  {path}")
            ...
            if mismatches:
                log('WARN', f"E002 {path}: {mismatches}")
        else:
            log('WARN', f"E001 {path}: file exists but 0 keys parsed")
...
def scan_folder(path, parent_hidden=False, inherited_spacing=None, inherited_zoom=None, inherited_keywords=None, inherited_reverse=None, inherited_sprite_bg=None, inherited_sprite_bg_opacity=None, inherited_sprite_bg_crop=None, inherited_sprite_bg_crop_opacity=None, ignore=None):
    ...
    if path.is_dir():
        ...
        if texts and images:
            log('WARN', f"W001 {path}: {[Path(p).name for p in images]}")
            images = []
...
log('INFO', f"TARGET_ROOT={TARGET_ROOT} exists={target_root.exists()} is_dir={target_root.is_dir() if target_root.exists() else 'N/A'}")
if target_root.exists() and target_root.is_dir():
    ...
    _tally(root)
    log('INFO', f"scan   done   images={_counts['images']} gifs={_counts['gifs']} html={_counts['html']}  ({time.time()-_t:.1f}s)")
else:
    log('ERR', f"E009 {TARGET_ROOT}")
    sys.exit(1)
...
_t = time.time()
log('INFO', "labels start")
manage_folder_labels(root, DEFAULTS)
log('INFO', f"labels done   ({time.time()-_t:.1f}s)")
...
def _convert_to_ktx2(png_path):
    ...
    try:
        ...
        if result.returncode != 0:
            log('ERR', f"E006 {png_path}: exit={result.returncode}")
            if VERBOSE:
                print(f"  stdout: {result.stdout}\n  stderr: {result.stderr}")
            raise RuntimeError(f"toktx failed with exit code {result.returncode}")
    except FileNotFoundError:
        log('ERR', "E005 install KTX-Software: https://github.com/KhronosGroup/KTX-Software/releases")
        raise
    return ktx2_path
...
def save_sheet_with_lod(sheet, sheet_idx, sprite_size, conf=None):
    ...
    log('INFO', f"sheet {sheet_idx} sprite={sprite_size} {sheet.width}px -> {lod1_size}px -> {lod2_size}px")
...
_t = time.time()
log('INFO', f"pack   start  {total_items} items")
if NO_IMAGES:
    _cb = 128
    ...
    if is_gif:
        try:
            ...
            if gif.n_frames > conf['MAX_GIF_FRAMES']:
                log('WARN', f"W002 {img_path}: {gif.n_frames} -> {frame_count}")
...
log('INFO', f"pack   done   {_counts['sheets']} sheets  {unique_slots} unique / {total_all} total  ({time.time()-_t:.1f}s)")
...
log('INFO', f"html   done   ({time.time()-_t:.1f}s)")
log('INFO', f"summary  images={_counts['images']} gifs={_counts['gifs']} html={_counts['html']} sheets={_counts['sheets']} errors={_counts['errors']} warnings={_counts['warnings']}  ({int(_elapsed // 60)}m {int(_elapsed % 60)}s)")
