...
import numpy as np
from image_processing import apply_filter, resize_image, DEFAULT_PIPELINE_ORDER, RESIZE_METHODS
from label_processing import create_label_image, load_custom_font, is_label_image

HIDDEN_SYSTEM_FOLDERS = {'venv', '.git', '__pycache__', 'node_modules'}
NON_PROCESSABLE_FOLDERS = {'fonts', 'images', 'spritesheets', 'backup', 'geo'}
HIDDEN_WORKING_FOLDERS = {'workfolder', 'ORIGINAL'}
SCAN_IGNORE_FOLDERS = HIDDEN_SYSTEM_FOLDERS | NON_PROCESSABLE_FOLDERS | HIDDEN_WORKING_FOLDERS
...
CODES = {
    'E001': '.custom_processing parse failed',
    'E002': '.custom_processing type mismatch',
    'E003': '.keywords read failed',
    'E004': 'dotfile read failed',
    'E005': 'toktx not found',
    'E006': 'toktx nonzero exit',
    'E007': 'GIF processing failed',
    'E008': 'image processing failed',
    'E009': 'TARGET_ROOT missing or not a directory',
    'W001': 'HTML-sibling images excluded',
    'W002': 'GIF frames truncated at MAX_GIF_FRAMES',
}

_counts = {'images': 0, 'gifs': 0, 'html': 0, 'sheets': 0, 'errors': 0, 'warnings': 0}

def _emit(level, code, ctx='', detail=''):
    if code not in CODES: raise KeyError(f"unknown log code: {code}")
    prefix = {'WARN': '[WARN] ', 'ERR': '[ERR]  '}[level]
    _counts['errors' if level == 'ERR' else 'warnings'] += 1
    parts = [prefix + code]
    if ctx: parts.append(str(ctx))
    if detail: parts.append(str(detail))
    print('  '.join(parts))

def warn(code, ctx='', detail=''): _emit('WARN', code, ctx, detail)
def err(code, ctx='', detail=''): _emit('ERR', code, ctx, detail)
...
DEFAULTS = {
    ...
    'COLOR_TO_TRANSPARENT_THRESHOLD': 0,
    ...
    'ALPHA_OUTLINE': False,
    'ALPHA_OUTLINE_THICKNESS': -3,
    'ALPHA_OUTLINE_COLOR': [0, 0, 0],
    'ALPHA_OUTLINE_LERPCOLOR': 'newcolor',
    ...
    'COLORIZE': False,
    'COLORIZE_COLOR': [255, 255, 255],
    'COLORIZE_THRESHOLD': 0,

    'COLOR_REPLACE': False,
    'COLOR_REPLACE_SRC': [0, 0, 0],
    'COLOR_REPLACE_THRESHOLD': 100,
    'COLOR_REPLACE_DST': [0, 0, 0],
    ...
    'RECT_MODE': 'overlay',
    'RECT_FILL': False,
    'RECT_FILL_COLOR': [255, 255, 255],
    'RECT_BORDER': 0,
    'RECT_BORDER_COLOR': [255, 255, 255],
    'RECT_CUT_BORDER': 0,
    'RECT_CUT_BORDER_COLOR': [255, 255, 255],
    'RECT_ROUNDNESS': 0.0,
    ...
    'DETECT_BOX_COLOR': [255, 255, 0],
    'DETECT_BOX_THICKNESS': 2,
    'DETECT_LABEL': True,
    'DETECT_LABEL_COLOR': [0, 0, 0],
    'DETECT_LABEL_BG': [255, 255, 0],
    'DETECT_LABEL_SCALE': 0.5,
    'DETECT_CLASSES': '',

    'MEDIAPIPE_MODEL_DIR': '',
    'MOBILENETSSD_MODEL_DIR': '',

    'PIPELINE_ORDER': ['sharpen', 'blur', 'contrast', 'exposure', 'gamma', 'hue', 'saturation', 'vibrance', 'color_to_transparent', 'alpha_outline', 'dither', 'invert', 'pixelate', 'fill', 'colorize', 'color_replace', 'rectangle', 'line', 'edge_outline', 'grain', 'contour', 'pose', 'face', 'detect'],
    ...
    'IMAGE_SETTINGS_COLOR': [255, 255, 255],    
    'IMAGE_SETTINGS_FONTSIZE': 3,      
    'IMAGE_SETTINGS_CUSTOMTEXT': "",  
    'IMAGE_SETTINGS_MODE': 'custom_text',
...
KTX2_ENCODE = 'uastc'   # 'uastc' (high quality, larger files) or 'etc1s' (smaller files, lower quality). Default: 'uastc'
KTX2_QUALITY = 3        # uastc: 0-4 (0=fastest, 4=best). etc1s: 1-255 (higher=better). Default: 2
...
ZOOM_VALUE = 0.4
SPRITE_BG = True
SPRITE_BG_OPACITY = 0.5
SPRITE_BG_CROP = True
SPRITE_BG_CROP_OPACITY = 1.0

RANDOM_IMAGE_ROTATION = False
DISCRETE_IMAGE_ROTATION = False
DISCRETE_IMAGE_ROTATION_ANGLE = 90
RANDOM_STACK_ROTATION = False
DISCRETE_STACK_ROTATION = False
DISCRETE_STACK_ROTATION_ANGLE = 90
RANDOM_GROUP_ROTATION = False
DISCRETE_GROUP_ROTATION = False
DISCRETE_GROUP_ROTATION_ANGLE = 90
IMAGE_ROTATION_PERCENTAGE = 100
STACK_ROTATION_PERCENTAGE = 100
GROUP_ROTATION_PERCENTAGE = 100
                        ...
                        try:
                            ...
                            if value.lower() == 'true':
                                ...
                                try:
                                    ...
                                    new_config[key] = _json.loads(value)
                                except:
                                    new_config[key] = value
                        parsed_keys[key] = new_config[key]
        except Exception as e:
            warn('E001', path, e)
        if parsed_keys:
            if VERBOSE:
                info(f".custom_processing loaded {len(parsed_keys)} keys  {path}")
                for k, v in parsed_keys.items():
                    def_v = DEFAULTS.get(k)
                    changed = v != def_v
                    print(f"    {'*' if changed else ' '} {k} = {repr(v)[:80]}  (default={repr(def_v)[:40]})")
            mismatches = {k: f"{type(DEFAULTS[k]).__name__}->{type(v).__name__} val={repr(v)[:60]}" for k, v in parsed_keys.items() if type(v) != type(DEFAULTS[k])}
            if mismatches:
                warn('E002', path, mismatches)
        else:
            warn('E001', path, 'file exists but 0 keys parsed')
...
def scan_folder(path, parent_hidden=False, inherited_spacing=None, inherited_zoom=None, inherited_keywords=None, inherited_reverse=None, inherited_sprite_bg=None, inherited_sprite_bg_opacity=None, inherited_sprite_bg_crop=None, inherited_sprite_bg_crop_opacity=None, inherited_random_image_rotation=None, inherited_discrete_image_rotation=None, inherited_discrete_image_rotation_angle=None, inherited_random_stack_rotation=None, inherited_discrete_stack_rotation=None, inherited_discrete_stack_rotation_angle=None, inherited_random_group_rotation=None, inherited_discrete_group_rotation=None, inherited_discrete_group_rotation_angle=None, inherited_image_rotation_percentage=None, inherited_stack_rotation_percentage=None, inherited_group_rotation_percentage=None, ignore=None):
    if ignore is None:
        ignore = SCAN_IGNORE_FOLDERS
    ...
    manual_sprite_bg_crop = inherited_sprite_bg_crop
    manual_sprite_bg_crop_opacity = inherited_sprite_bg_crop_opacity
    manual_random_image_rotation = inherited_random_image_rotation
    manual_discrete_image_rotation = inherited_discrete_image_rotation
    manual_discrete_image_rotation_angle = inherited_discrete_image_rotation_angle
    manual_random_stack_rotation = inherited_random_stack_rotation
    manual_discrete_stack_rotation = inherited_discrete_stack_rotation
    manual_discrete_stack_rotation_angle = inherited_discrete_stack_rotation_angle
    manual_random_group_rotation = inherited_random_group_rotation
    manual_discrete_group_rotation = inherited_discrete_group_rotation
    manual_discrete_group_rotation_angle = inherited_discrete_group_rotation_angle
    manual_image_rotation_percentage = inherited_image_rotation_percentage
    manual_stack_rotation_percentage = inherited_stack_rotation_percentage
    manual_group_rotation_percentage = inherited_group_rotation_percentage
    ...
    if path.is_dir():
                    ...
                    if '=' in line:
                        ...
                        if key == 'SPRITE_BG_OPACITY':
                            try:
                                ...
                                pass
                        if key == 'SPRITE_BG_CROP':
                            try:
                                manual_sprite_bg_crop = ast.literal_eval(value)
                            except:
                                pass
                        if key == 'SPRITE_BG_CROP_OPACITY':
                            try:
                                manual_sprite_bg_crop_opacity = float(ast.literal_eval(value))
                            except:
                                pass
                        if key == 'RANDOM_IMAGE_ROTATION':
                            try:
                                manual_random_image_rotation = ast.literal_eval(value)
                            except:
                                pass
                        if key == 'DISCRETE_IMAGE_ROTATION':
                            try:
                                manual_discrete_image_rotation = ast.literal_eval(value)
                            except:
                                pass
                        if key == 'DISCRETE_IMAGE_ROTATION_ANGLE':
                            try:
                                manual_discrete_image_rotation_angle = int(ast.literal_eval(value))
                            except:
                                pass
                        if key == 'RANDOM_STACK_ROTATION':
                            try:
                                manual_random_stack_rotation = ast.literal_eval(value)
                            except:
                                pass
                        if key == 'DISCRETE_STACK_ROTATION':
                            try:
                                manual_discrete_stack_rotation = ast.literal_eval(value)
                            except:
                                pass
                        if key == 'DISCRETE_STACK_ROTATION_ANGLE':
                            try:
                                manual_discrete_stack_rotation_angle = int(ast.literal_eval(value))
                            except:
                                pass
                        if key == 'RANDOM_GROUP_ROTATION':
                            try:
                                manual_random_group_rotation = ast.literal_eval(value)
                            except:
                                pass
                        if key == 'DISCRETE_GROUP_ROTATION':
                            try:
                                manual_discrete_group_rotation = ast.literal_eval(value)
                            except:
                                pass
                        if key == 'DISCRETE_GROUP_ROTATION_ANGLE':
                            try:
                                manual_discrete_group_rotation_angle = int(ast.literal_eval(value))
                            except:
                                pass
                        if key == 'IMAGE_ROTATION_PERCENTAGE':
                            try:
                                manual_image_rotation_percentage = int(ast.literal_eval(value))
                            except:
                                pass
                        if key == 'STACK_ROTATION_PERCENTAGE':
                            try:
                                manual_stack_rotation_percentage = int(ast.literal_eval(value))
                            except:
                                pass
                        if key == 'GROUP_ROTATION_PERCENTAGE':
                            try:
                                manual_group_rotation_percentage = int(ast.literal_eval(value))
                            except:
                                pass
            except:
                pass
        ...
        for item in natsorted(path.iterdir(), key=lambda x: (not x.is_dir(), x.name)):
            ...
            if item.is_file():
                ...
                child = scan_folder(item, parent_hidden=is_hidden, inherited_spacing=manual_spacing, inherited_zoom=manual_zoom if zoom_propagate else None, inherited_keywords=keywords, inherited_reverse=manual_reverse, inherited_sprite_bg=manual_sprite_bg, inherited_sprite_bg_opacity=manual_sprite_bg_opacity, inherited_sprite_bg_crop=manual_sprite_bg_crop, inherited_sprite_bg_crop_opacity=manual_sprite_bg_crop_opacity, inherited_random_image_rotation=manual_random_image_rotation, inherited_discrete_image_rotation=manual_discrete_image_rotation, inherited_discrete_image_rotation_angle=manual_discrete_image_rotation_angle, inherited_random_stack_rotation=manual_random_stack_rotation, inherited_discrete_stack_rotation=manual_discrete_stack_rotation, inherited_discrete_stack_rotation_angle=manual_discrete_stack_rotation_angle, inherited_random_group_rotation=manual_random_group_rotation, inherited_discrete_group_rotation=manual_discrete_group_rotation, inherited_discrete_group_rotation_angle=manual_discrete_group_rotation_angle, inherited_image_rotation_percentage=manual_image_rotation_percentage, inherited_stack_rotation_percentage=manual_stack_rotation_percentage, inherited_group_rotation_percentage=manual_group_rotation_percentage, ignore=ignore)
                if child:
                    children.append(child)
        
        if texts and images:
            warn('W001', path, [Path(p).name for p in images])
            images = []
    ...
    result = {
        ...
        'sbc': manual_sprite_bg_crop,
        'sbco': manual_sprite_bg_crop_opacity,
        'rir': manual_random_image_rotation,
        'dir': manual_discrete_image_rotation,
        'dira': manual_discrete_image_rotation_angle,
        'rsr': manual_random_stack_rotation,
        'dsr': manual_discrete_stack_rotation,
        'dsra': manual_discrete_stack_rotation_angle,
        'rgr': manual_random_group_rotation,
        'dgr': manual_discrete_group_rotation,
        'dgra': manual_discrete_group_rotation_angle,
        'irp': manual_image_rotation_percentage,
        'srp': manual_stack_rotation_percentage,
        'grp': manual_group_rotation_percentage,
...
TARGET_ROOT = 'archiGrad.io'

target_root = Path(TARGET_ROOT)
info(f"TARGET_ROOT={TARGET_ROOT} exists={target_root.exists()} is_dir={target_root.is_dir() if target_root.exists() else 'N/A'}")
if target_root.exists() and target_root.is_dir():
    ...
    def _tally(node):
        _counts['html'] += len(node['ot'])
        for p in node['oi']:
            if p.lower().endswith('.gif'): _counts['gifs'] += 1
            else: _counts['images'] += 1
        for c in node['children']: _tally(c)
    _tally(root)
    info(f"scan   done   images={_counts['images']} gifs={_counts['gifs']} html={_counts['html']}  ({time.time()-_t:.1f}s)")
else:
    err('E009', TARGET_ROOT)
    sys.exit(1)
...
def manage_folder_labels(node, parent_config):
    ...
    if should_generate and has_real_images:
        color = current_config.get('IMAGE_SETTINGS_COLOR', [255, 255, 255])
        ...
        settings_mode = current_config.get('IMAGE_SETTINGS_MODE', 'custom_text')
        ...
        # -------------------------------------------------
        # PRIORITY LOGIC (IMAGE_SETTINGS_MODE)
        # 1) .custom_processing -> show nearest .custom_processing content
        # 2) .keywords -> show inherited keywords (node['sk'])
        # 3) custom_text + non-empty -> use IMAGE_SETTINGS_CUSTOMTEXT
        # 4) fallback -> dotfile -> folder name
        # -------------------------------------------------
        if settings_mode == '.custom_processing':
            cp_file = None
            search = node_path
            while search != search.parent:
                candidate = search / '.custom_processing'
                if candidate.exists():
                    cp_file = candidate
                    break
                search = search.parent
            if cp_file:
                cp_lines = [l.strip() for l in cp_file.read_text(encoding='utf-8').splitlines() if l.strip()]
                top_title = ""
                top_body = cp_lines
                bottom_title = ""
                bottom_body = cp_lines
            else:
                top_title = folder_name
                top_body = ["(no .cp)"]
                bottom_title = folder_name
                bottom_body = ["(no .cp)"]
        elif settings_mode == '.keywords':
            kw_lines = node.get('sk', [])
            if kw_lines:
                top_title = ""
                top_body = kw_lines
                bottom_title = ""
                bottom_body = kw_lines
            else:
                top_title = folder_name
                top_body = ["(no .keywords)"]
                bottom_title = folder_name
                bottom_body = ["(no .keywords)"]
...
info(f"labels done   ({time.time()-_t:.1f}s)")
...
def _is_fully_transparent(img, threshold=0.001):
...
def _convert_to_ktx2(png_path):
    ...
    try:
        ...
        if result.returncode != 0:
            err('E006', png_path, f"exit={result.returncode}")
            if VERBOSE:
                print(f"  stdout: {result.stdout}\n  stderr: {result.stderr}")
            raise RuntimeError(f"toktx failed with exit code {result.returncode}")
    except FileNotFoundError:
        err('E005', '', 'install KTX-Software: https://github.com/KhronosGroup/KTX-Software/releases')
        raise
    return ktx2_path
...
def save_sheet_with_lod(sheet, sheet_idx, sprite_size, conf=None):
    ...
    info(f"sheet {sheet_idx} sprite={sprite_size} {sheet.width}px -> {lod1_size}px -> {lod2_size}px")
...
_t = time.time()
info(f"pack   start  {total_items} items")
if NO_IMAGES:
    _cb = 128
    placeholder = Image.new('RGBA', (_cb, _cb), (0, 0, 0, 255))
    _px = placeholder.load()
    for _y in range(_cb):
        for _x in range(_cb):
            if (_x + _y) % 2: _px[_x, _y] = (255, 0, 255, 255)
    placeholder = placeholder.resize((SPRITESHEET_SIZE, SPRITESHEET_SIZE), Image.NEAREST)
    save_sheet_with_lod(placeholder, 0, DEFAULTS['SPRITE_SIZE'], DEFAULTS)
    for item in all_image_items:
        ts = item['conf']['SPRITE_SIZE']
        sprite_data[item['path']] = {'ss': lod_paths(0), 'idx': 0, 'w': ts, 'h': ts, 'sz': ts, 'path': item['path']}
    all_image_items = []
for i, item in enumerate(all_image_items):
    ...
    if is_gif:
        try:
            ...
            if gif.n_frames > conf['MAX_GIF_FRAMES']:
                warn('W002', img_path, f"{gif.n_frames} -> {frame_count}")
            ...
            for frame_idx in range(frame_count):
                ...
                _scope = conf.get('PROCESSING_SCOPE', 'all')
                if _scope == 'all' or (_scope == 'stack' and not is_label_image(Path(img_path).name)) or (_scope == 'labels' and is_label_image(Path(img_path).name)):
                    frame = apply_filter(frame, conf)
                frames.append(frame)

            new_hashes = [hash(f.tobytes()) for f in frames if not _is_fully_transparent(f, conf.get('TRANSPARENCY_THRESHOLD', 0.001))]
            ...
            fm = []
            for frame in frames:
                if _is_fully_transparent(frame, conf.get('TRANSPARENCY_THRESHOLD', 0.001)):
            ...
            sprite_data[img_path] = {
                ...
                'fm': fm,
                'anim': True,
                'w': frames[0].width if frames else target_size,
                'h': frames[0].height if frames else target_size,
                'sz': target_size,
                'gd': gif_delay_ms,
                'path': img_path
            }
            ...
            if VERBOSE:
                ...
                print(f"  conf keys/types: { {k: (type(v).__name__, repr(v)[:80]) for k,v in conf.items()} }")
    else:
        try:
            ...
            if _is_fully_transparent(img, conf.get('TRANSPARENCY_THRESHOLD', 0.001)):
...
info(f"pack   done   {_counts['sheets']} sheets  {unique_slots} unique / {total_all} total  ({time.time()-_t:.1f}s)")
...
sprite_config = {
    ...
    'sprite_bg_crop': SPRITE_BG_CROP,
    'sprite_bg_crop_opacity': SPRITE_BG_CROP_OPACITY,
    'pipeline_order': DEFAULTS.get('PIPELINE_ORDER', DEFAULT_PIPELINE_ORDER),

    'random_image_rotation': RANDOM_IMAGE_ROTATION,
    'discrete_image_rotation': DISCRETE_IMAGE_ROTATION,
    'discrete_image_rotation_angle': DISCRETE_IMAGE_ROTATION_ANGLE,
    'random_stack_rotation': RANDOM_STACK_ROTATION,
    'discrete_stack_rotation': DISCRETE_STACK_ROTATION,
    'discrete_stack_rotation_angle': DISCRETE_STACK_ROTATION_ANGLE,
    'random_group_rotation': RANDOM_GROUP_ROTATION,
    'discrete_group_rotation': DISCRETE_GROUP_ROTATION,
    'discrete_group_rotation_angle': DISCRETE_GROUP_ROTATION_ANGLE,
    'image_rotation_percentage': IMAGE_ROTATION_PERCENTAGE,
    'stack_rotation_percentage': STACK_ROTATION_PERCENTAGE,
    'group_rotation_percentage': GROUP_ROTATION_PERCENTAGE,
}
...
<div id="tree">
    ...
    <div id="debug-section" style="display:none;">
        <div id="debug-section-title">debug (d)</div>
        <label class="debug-row">
            <input type="checkbox" id="debug-stats"> stats
        </label>
    </div>
</div>
<div id="content"></div>
...
// Debug section: 'd' toggles, checkbox controls stats visibility
(() => {{
    const section = document.getElementById('debug-section');
    const statsCheckbox = document.getElementById('debug-stats');
    statsCheckbox.checked = localStorage.getItem('debug_show_stats') !== '0';
    ...
    statsCheckbox.addEventListener('change', () => {{
        const show = statsCheckbox.checked;
        localStorage.setItem('debug_show_stats', show ? '1' : '0');
        (window._statsInstances || []).forEach(dom => {{ dom.style.display = show ? '' : 'none'; }});
    }});
}})();
...
function rebaseHtmlSrc(html, htmlPath) {{
    const base = htmlPath.substring(0, htmlPath.lastIndexOf('/') + 1);
    if (!base) return html;
    return html.replace(/(src\s*=\s*["'])(?!https?:\/\/|data:|\/|#)([^"']+)(["'])/gi, (m, pre, val, post) => pre + base + val + post);
}}

function assembleHtmlParts(parts, paths) {{
    const total = parts.length;
    return parts.map((part, i) => {{
        const rebased = rebaseHtmlSrc(part, paths[i]);
        ...
        return `<div class=\"text-page\"><div class=\"text-page-inner\">${{rebased}}</div>${{pageLabel}}</div>${{separator}}`;
    }}).join('');
}}
...
function createAnimInstancedMaterial(ssArray, frameMapTex, frameMapWidth) {{
    ...
    const mat = new THREE.ShaderMaterial({{
        ...
        vertexShader: `
            ...
            attribute float spriteW;
            attribute float spriteH;
            ...
            void main() {{
                ...
                float uW = spriteW / spritesheetSize;
                float vH = spriteH / spritesheetSize;
                float u0 = col * sz;
                float v0 = 1.0 - row * sz - vH;
                vUv = vec2(u0 + uv.x * uW, v0 + uv.y * vH);
...
function createSpritesheetBg(container, images, SS, scene, renderer, opacity, cropConfig) {{
    const cropEnabled = cropConfig && cropConfig.enabled;
    const cropOpacity = cropConfig ? cropConfig.opacity : 1.0;
    const slotMap = [];
    const keys = new Set();
    ...
    if (cropEnabled) {{
        ...
        preview.className = 'ss-crop-preview';
        if (cropOpacity < 1) preview.style.opacity = cropOpacity;
    ...
    const hitId = (hit) => {{
        ...
        if (obj.isInstancedMesh && hit.instanceId != null) return 'i:' + obj.uuid + ':' + hit.instanceId;
        return 'f:' + obj.uuid + ':' + Math.floor(hit.faceIndex / 2);
    }};

    const cropStyle = cropEnabled ? getComputedStyle(preview) : null;
    const cropPct = cropStyle ? parseFloat(cropStyle.getPropertyValue('--ss-crop-size') || '50') / 100 : 0.5;
    const cropNarrowPx = cropStyle ? parseFloat(cropStyle.getPropertyValue('--ss-crop-narrow-threshold') || '300') : 300;

    const positionPreview = (slot, imgEl) => {{
        if (!preview || !imgEl) return;
        const ir = imgEl.getBoundingClientRect();
        const cr = container.getBoundingClientRect();
        const narrow = cr.width < cropNarrowPx || cr.height < cropNarrowPx;
        const target = narrow ? container : el;
        if (preview.parentNode !== target) {{ preview.remove(); target.appendChild(preview); }}
        preview.classList.toggle('ss-crop-narrow', narrow);
        const refSz = narrow ? Math.min(cr.width, cr.height) : ir.width;
        const displaySz = refSz * cropPct;
        if (narrow) {{
            preview.style.left = '50%';
            preview.style.top = '50%';
        }} else {{
            const er = el.getBoundingClientRect();
            preview.style.left = (ir.left - er.left + ir.width / 2) + 'px';
            preview.style.top = (ir.top - er.top + ir.height / 2) + 'px';
        }}
        preview.style.width = displaySz + 'px';
        preview.style.height = displaySz + 'px';
        const sc = displaySz / slot.pw;
        preview.style.backgroundImage = 'url(' + imgEl.src + ')';
        preview.style.backgroundSize = (SS * sc) + 'px ' + (SS * sc) + 'px';
        preview.style.backgroundPosition = (-slot.px * sc) + 'px ' + (-slot.py * sc) + 'px';
    ...
    let lastHitId = null;
    const onDown = (e) => {{ mouseDownPos = {{ x: e.clientX, y: e.clientY }}; }};
    const onClick = (e) => {{
        if (!cropEnabled) return;
        if (mouseDownPos && (Math.abs(e.clientX - mouseDownPos.x) > 4 || Math.abs(e.clientY - mouseDownPos.y) > 4)) return;
        if (!currentSlot) {{ hidePreview(); return; }}
        if (lastHitId && pinnedSlot && lastHitId === pinnedSlot._hitId) {{ hidePreview(); return; }}
        pinnedSlot = {{ ...currentSlot, _hitId: lastHitId }};
        pinnedAnim = currentAnim ? {{ ...currentAnim }} : null;
        const imgEl = wrap.querySelector('img[data-ss-key="' + currentSlot.key + '"]');
        positionPreview(currentSlot, imgEl);
    }};
    ...
    function update(camera, time) {{
        ...
        if (hits.length > 0) {{
            ...
            if (isInstanced && hit.instanceId != null && hit.object._animInstances) {{
                ...
                if (inst) {{
                    ...
                    currentAnim = {{ key, sz, spr, gd: imgData.gd, fm: imgData.fm }};
                    dirty = true;
                }}
            }} else if (hit.uv) {{
            ...
            currentSlot = (slot && imgEl) ? {{ key: slot.key, px: slot.px, py: slot.py, pw: slot.pw, ph: slot.ph }} : null;
            if (slot && imgEl) {{
                const ir = imgEl.getBoundingClientRect();
            ...
            if (!wireframe) {{
                ...
                const wMat = new LineMaterial({{ color: "blue", linewidth: 2 }});
        ...
        if (pinnedAnim && preview && preview.style.display !== 'none') {{
            const a = pinnedAnim;
            const frame = Math.floor(time / a.gd) % a.fm.length;
            const idx = a.fm[frame];
            const px = (idx % a.spr) * a.sz;
            const py = Math.floor(idx / a.spr) * a.sz;
            const imgEl = wrap.querySelector('img[data-ss-key="' + a.key + '"]');
            if (imgEl) {{
                const displaySz = preview.offsetWidth;
                const sc = displaySz / a.sz;
                preview.style.backgroundPosition = (-px * sc) + 'px ' + (-py * sc) + 'px';
            }}
            dirty = true;
        }}
    }}
...
async function createThreeScene(container, images, node, highlightPath) {{
    ...
    const RANDOM_IMAGE_ROTATION = node.rir != null ? node.rir : spriteConfig.random_image_rotation;
    const DISCRETE_IMAGE_ROTATION = node.dir != null ? node.dir : spriteConfig.discrete_image_rotation;
    const DISCRETE_IMAGE_ROTATION_ANGLE = node.dira != null ? node.dira : spriteConfig.discrete_image_rotation_angle;
    const RANDOM_STACK_ROTATION = node.rsr != null ? node.rsr : spriteConfig.random_stack_rotation;
    const DISCRETE_STACK_ROTATION = node.dsr != null ? node.dsr : spriteConfig.discrete_stack_rotation;
    const DISCRETE_STACK_ROTATION_ANGLE = node.dsra != null ? node.dsra : spriteConfig.discrete_stack_rotation_angle;
    const RANDOM_GROUP_ROTATION = node.rgr != null ? node.rgr : spriteConfig.random_group_rotation;
    const DISCRETE_GROUP_ROTATION = node.dgr != null ? node.dgr : spriteConfig.discrete_group_rotation;
    const DISCRETE_GROUP_ROTATION_ANGLE = node.dgra != null ? node.dgra : spriteConfig.discrete_group_rotation_angle;
    const IMAGE_ROTATION_PERCENTAGE = node.irp != null ? node.irp : spriteConfig.image_rotation_percentage;
    const STACK_ROTATION_PERCENTAGE = node.srp != null ? node.srp : spriteConfig.stack_rotation_percentage;
    const GROUP_ROTATION_PERCENTAGE = node.grp != null ? node.grp : spriteConfig.group_rotation_percentage;

    const seededRotFromPath = (path) => {{
        const hash = path.split('').reduce((a, c) => a + c.charCodeAt(0), 0);
        let state = hash * SEED;
        state = ((state * 1664525 + 1013904223) % 4294967296 + 4294967296) % 4294967296;
        return Math.floor((state / 4294967296) * 4);
    }};
    const seededFloatFromPath = (path, salt) => {{
        const hash = (salt + path).split('').reduce((a, c) => a + c.charCodeAt(0), 0);
        let state = hash * SEED;
        state = ((state * 1664525 + 1013904223) % 4294967296 + 4294967296) % 4294967296;
        return state / 4294967296;
    }};
    const _rollsGate = (pct, path, salt) => {{
        if (pct >= 100) return true;
        if (pct <= 0) return false;
        return seededFloatFromPath(path, salt) * 100 < pct;
    }};
    const imageRotationAngle = (imgPath) => {{
        if (!_rollsGate(IMAGE_ROTATION_PERCENTAGE, imgPath, 'img-chance:')) return 0;
        if (RANDOM_IMAGE_ROTATION) return seededRotFromPath('img:' + imgPath) * Math.PI / 2;
        if (DISCRETE_IMAGE_ROTATION) return DISCRETE_IMAGE_ROTATION_ANGLE * Math.PI / 180;
        return 0;
    }};
    const stackRotationAngle = (folderPath) => {{
        if (!_rollsGate(STACK_ROTATION_PERCENTAGE, folderPath, 'stack-chance:')) return 0;
        if (RANDOM_STACK_ROTATION) return seededRotFromPath('stack:' + folderPath) * Math.PI / 2;
        if (DISCRETE_STACK_ROTATION) return DISCRETE_STACK_ROTATION_ANGLE * Math.PI / 180;
        return 0;
    }};
    const groupRotationAngle = (groupKey) => {{
        if (!_rollsGate(GROUP_ROTATION_PERCENTAGE, groupKey, 'group-chance:')) return 0;
        if (RANDOM_GROUP_ROTATION) return seededRotFromPath('group:' + groupKey) * Math.PI / 2;
        if (DISCRETE_GROUP_ROTATION) return DISCRETE_GROUP_ROTATION_ANGLE * Math.PI / 180;
        return 0;
    }};
 ...
 let minX = Infinity, maxX = -Infinity;
    ...
    const registerGroup = (groupKey, groupFolders) => {{
        if (groupTransforms[groupKey] != null) return;
        if (!groupFolders.length) return;
        let cx = 0, cz = 0, n = 0;
        for (const f of groupFolders) {{
            const p = folderPositions[f];
            if (!p) continue;
            cx += p.x; cz += p.z; n++;
        }}
        if (n === 0) return;
        cx /= n; cz /= n;
        const ang = groupRotationAngle(groupKey);
        if (ang === 0) {{ groupTransforms[groupKey] = {{ angle: 0, mat: null }}; return; }}
        const mat = new THREE.Matrix4().makeTranslation(cx, 0, cz);
        mat.multiply(new THREE.Matrix4().makeRotationY(ang));
        mat.multiply(new THREE.Matrix4().makeTranslation(-cx, 0, -cz));
        groupTransforms[groupKey] = {{ angle: ang, mat }};
    }};
    if (node.grid_layout) {{
        const gk = 'outer:' + (node.path || '');
        folders.forEach(f => {{ folderToGroup[f] = gk; }});
        registerGroup(gk, folders);
    }} else if (gridGroups && gridGroups.length > 0) {{
        gridGroups.forEach(g => {{
            const isReal = (g.cols > 1 || g.rows > 1);
            if (!isReal) return;
            const gk = 'inner:' + g.path;
            g.folders.forEach(f => {{ folderToGroup[f] = gk; }});
            registerGroup(gk, g.folders);
        }});
    }}
    ...
    const margin = 5;
    const paddedWidth = geomWidth + margin;
    const paddedDepth = geomDepth + margin;

    const aspectRatio = container.clientWidth / container.clientHeight;
    const sceneAspect = paddedWidth / paddedDepth;

    // FIT LOGIC (contain): fit whichever dimension is the binding constraint
    // given the scene's own aspect vs the container's aspect.
    const fitFrustumSize = sceneAspect > aspectRatio
        ? paddedWidth / aspectRatio   // scene wider than container -> width-limited
        : paddedDepth;                // scene taller than container -> height-limited
    ...
    const spriteBgCrop = node.sbc != null ? node.sbc : spriteConfig.sprite_bg_crop;
    const spriteBgCropOpacity = node.sbco != null ? node.sbco : spriteConfig.sprite_bg_crop_opacity;
    const needsSsBg = isLastStack && (spriteBgEnabled || spriteBgCrop);
    const ssBg = needsSsBg ? createSpritesheetBg(container, images, SPRITESHEET_SIZE, scene, renderer, spriteBgEnabled ? spriteBgOpacity : 0, {{ enabled: spriteBgCrop, opacity: spriteBgCropOpacity }}) : null;
...
const updateCount = () => {{
    ...
    const loadingPromise = (async () => {{
        ...
        for (let stackIdx = 0; stackIdx < folders.length; stackIdx++) {{
            ...
            for (let i = 0; i < stackImages.length; i++) {{
                ...
                const imgAng = imageRotationAngle(imgData.path || (folderName + '/' + i));
                const stackAng = stackRotationAngle(folderName);
                const localAng = imgAng + stackAng;
                if (localAng !== 0) {{
                    const rotY = new THREE.Matrix4().makeRotationY(localAng);
                    const t = new THREE.Matrix4().makeTranslation(xPos, 0, zPos);
                    const tInv = new THREE.Matrix4().makeTranslation(-xPos, 0, -zPos);
                    matrix.premultiply(tInv).premultiply(rotY).premultiply(t);
                }}
                const groupKey = folderToGroup[folderName];
                if (groupKey) {{
                    const gt = groupTransforms[groupKey];
                    if (gt && gt.mat) matrix.premultiply(gt.mat);
                }}
                ...
                if (imgData.anim) {{
                    ...
                    const v_start = 1 - ((sprite_row * currentSpriteSize) + imgData.h) / SPRITESHEET_SIZE;
        ...
        for (const [bKey, bucket] of Object.entries(animBuckets)) {{
            ...
            for (let i = 0; i < count; i++) {{
                ...
                widths[i] = inst.imgData.w;
                heights[i] = inst.imgData.h;
            ...
            geo.setAttribute('spriteW', new THREE.InstancedBufferAttribute(widths, 1));
            geo.setAttribute('spriteH', new THREE.InstancedBufferAttribute(heights, 1));
    ...
    if (localStorage.getItem('debug_show_stats') === '0') stats.dom.style.display = 'none';
    window._statsInstances = window._statsInstances || [];
    window._statsInstances.push(stats.dom);
...
info(f"html   done   ({time.time()-_t:.1f}s)")
info(f"summary  images={_counts['images']} gifs={_counts['gifs']} html={_counts['html']} sheets={_counts['sheets']} errors={_counts['errors']} warnings={_counts['warnings']}  ({int(_elapsed // 60)}m {int(_elapsed % 60)}s)")
if _counts['errors'] == 0 and _counts['warnings'] == 0:
